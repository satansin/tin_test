...
inline constexpr const char* kDefaultRsTreeBundleFilename = "combined.rstree";

enum class RsTreeFolderLoadSource { Bundle, PerFile };

struct LoadRsTreesFromFolderOptions {
  std::string bundle_filename = kDefaultRsTreeBundleFilename;
};
...
[[nodiscard]] std::optional<std::filesystem::path> find_rs_tree_bundle_in_directory(
    const std::filesystem::path& dir,
    std::string_view bundle_filename = kDefaultRsTreeBundleFilename);
...
[[nodiscard]] LoadRsTreesFromFolderResult load_rs_trees_from_folder(
    ...
    LoadRsTreesFromFolderOptions opts = {});
