...
std::optional<CompressPlyConfig> parse_compress_ply_config(const int argc, char* argv[]) {
  CompressPlyConfig config;
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    if (arg == "--input-dir" || arg == "-i") {
      ...
      config.max_meshes_per_bundle = static_cast<std::size_t>(std::stoull(need_value(arg.c_str())));
  ...
  if (config.input_dir.empty()) {
    throw std::runtime_error("compress requires --input-dir DIR (or a positional DIR)");
  }
